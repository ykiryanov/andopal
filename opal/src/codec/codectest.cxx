...
#include <codec/codectest.h>

// #include "ippdefs.h"

#include <codec/udplistener.h>

#ifdef P_IOS
#include "TargetConditionals.h"

//extern void ReadFromDocumentFile(const char* szFileName, void* bytes, unsigned size);
//unsigned GetDocumentFileSize(const char* szFileName);
extern void Yield(int ms);
#endif
...
{
public:
    static void yuv420PlaybackCallback(void* opaque, Ipp8u* pYUV, int frameWidth, int frameHeight)
    {
    ...
    static void pcmPlaybackCallback(void* opaque, Ipp8u* pBuffer, unsigned nBufSize)
    {
        Mpeg2TSListener* pThis = (Mpeg2TSListener*) opaque;
    ...
    static void onReceiveCallback(void* _opaque, u8* pData, uint nDataSize)
    {
        ...
        if(pThis->isStarted())
            pThis->m_tsParser.processNetData((Ipp8u*) pData, nDataSize);
    }
...
//        _pOutBuffer = new Ipp8u[_frameSize];
//
//        _pInBuffer = new Ipp8u[MAX_BUF_SIZE];
//        _in.SetBufferPointer(_pInBuffer, MAX_BUF_SIZE);
//    }
//
//    bool Init()
//    {
...
//        {
//            printf("_Decoder.Init =>%d\n", _status);
//            return false;
//        }
//
//        _status =  _Decoder.GetInfo(&params);
//        if (_status != UMC::UMC_OK) {
//            printf("_Decoder.GetInfo =>%d\n", _status);
//            return false;
//        }
//
...
//
//
//    void decode(Ipp8u* pData, uint nDataSize)
//    {
//        _in.Reset();
//        memcpy(_pInBuffer, pData, nDataSize);
//        _in.SetDataSize(nDataSize);
//
//        if (!_bInit && !Init())
//            return;
//
...
//
//    if (pDec)
//        pDec->decode(pData, nDataSize);
//}
//
//void DispatchPacket(Ipp8u* data);
//void processData(Ipp8u* data, Ipp32u dataSize);
//
...
//static Ipp8u   s_iPid = 0;
//Ipp8u   s_buf[MAX_BUF_SIZE];
//static Ipp32u s_bufSize=0;
//static Ipp32u s_InFrameSize = 0;
//static int s_Frame = 0;
//
//inline Ipp16u GET_16U(Ipp8u* p) { return (((Ipp16u)p[0])<<8) | p[1];}
//
//void ShowFrame()
//{
//    if (s_bufSize > 0)
...
//#if 0
//        g_Frame = s_buf;
//        g_FrameSize = s_bufSize;
//
//        while(g_FrameSize)
//            PThread::Current()->Sleep(5);
//
//#else
//        decode(s_buf, s_bufSize);
//#endif
//    }
//
//    g_FrameSize = 0;
...
//}
//
//int ParsePSE(Ipp8u* data, int iPos)
//{
...
//
//    Ipp8u* pesPacket = data + iPos;
//
//    if (pesPacket[0] != 0 || pesPacket[1] != 0 || pesPacket[2] != 1)
//        return UMC_ERR_INVALID_STREAM;
//
//    if (pesPacket[3] != 0x0e0) //!IS_PES_PACKET(pesPacket[3]))
//        return UMC_ERR_INVALID_STREAM;
//
//
//    uint PES_packet_len = GET_16U(&pesPacket[4]);
//
//    //    Ipp32u PTS_DTS_flag = (pesPacket[iInnerPos + 7] >> 6) & 0x03;
//    Ipp32s PES_header_data_length = pesPacket[8];
//
//    s_InFrameSize = PES_packet_len - (PES_header_data_length + 3);
//
//
...
//
//    s_bufSize = PACKET_SIZE - iPos;
//    memcpy(s_buf, data + iPos, s_bufSize);
//
//    return 0;
//}
//
...
//    int iPos = 0;
//
//    Ipp8u sync_byte = data[0];
//    Ipp8u bPreStart = ((data[1]>>6) & 0x01);
//    Ipp16u iPid = ((data[1] & 0x1f) << 8) + data[2];
//    Ipp8u scrambling_ctrl = (data[3] >> 6) & 0x03;
//    Ipp8u adapt_field_ctrl = (data[3] >> 4) & 0x03;
//    Ipp8u adapt_field_len = data[4];
//
//    if (ID_TS_SYNC != sync_byte || scrambling_ctrl > 1 || ID_TS_NULL == iPid ||
...
//
//    if (!bPreStart) {
//        uint sz = PACKET_SIZE - iPos;
//        memcpy(s_buf+s_bufSize, data + iPos, sz);
//        s_bufSize += sz;
//
//        if ((s_InFrameSize > 0) && (s_bufSize >= s_InFrameSize))
//        {
//            printf("s_bufSize=%d sz=%d s_InFrameSize=%d\n", s_bufSize, sz, s_InFrameSize);
//            s_bufSize = s_InFrameSize;
//            ShowFrame();
//        }
//    } else {
//        if (s_bufSize > 0) {
//            printf("s_bufSize=%d s_InFrameSize=%d\n", s_bufSize, s_InFrameSize);
//            ShowFrame();
//        }
//        if (ParsePSE(data, iPos) == 0)
//            s_iPid = iPid;
