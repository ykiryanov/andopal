/*
 * H.264 Plugin codec for Dinsk BoneLib
 *
 * Copyright (c) 2010
 *
 * The Initial Developer of the Original Code is Dinsk, LLP.
 *
 * Contributor(s): Yuri Kiryanov, yuri@dinsk.net, Sergey Bezruchkin, sergey@dinsk.net
 *
 */

#include "ipph264codec.h"
...
{
    PTRACE(1, DINSK_CODEC_LOG, "H264\tDecoder\tDecoder successfully opened");

    if (_status = UMC::H264VideoDecoder::Init(&params) != UMC::UMC_OK) {
        CloseCodec();
    }
}
...
static int adjust_bitrate_to_level (unsigned & targetBitrate, unsigned level, int idx = -1)
{
  ...
  if (idx == -1) { 
    ...
    if (!h264_levels[i].level_idc) {
      PTRACE(1, DINSK_CODEC_LOG, "H264\tCap\tIllegal Level negotiated");
      return 0;
    }
  }
  else
...
// Correct Target Bitrate
  PTRACE(4, DINSK_CODEC_LOG, "H264\tCap\tBitrate: " << targetBitrate << "(" << h264_levels[i].bitrate << ")");
...
// Correct max. number of macroblocks per frame
  ...
  unsigned j = 0;
  PTRACE(4, DINSK_CODEC_LOG, "H264\tCap\tFrame Size: " << nbMBsPerFrame << "(" << h264_levels[i].frame_size << ")");
  if    ( (nbMBsPerFrame          > h264_levels[i].frame_size)
    ...
    if (!h264_resolutions[j].width) {
      PTRACE(1, DINSK_CODEC_LOG, "H264\tCap\tNo Resolution found that has number of macroblocks <=" << h264_levels[i].frame_size);
      return 0;
    }
    else {
...
// Correct macroblocks per second
  ...
  PTRACE(4, DINSK_CODEC_LOG, "H264\tCap\tMB/s: " << nbMBsPerSecond << "(" << h264_levels[i].mbps << ")");
...
static int to_normalised_options(const struct PluginCodec_Definition *, void *, const char *, void * parm, unsigned * parmLen)
{
  ...
  PTRACE(4, DINSK_CODEC_LOG, "H264\tCap\tProfile and Level: " << profile << ";" << constraints << ";" << level);
...
static int merge_profile_level_h264(char ** result, const char * dst, const char * src)
{
  ...
  *result = strdup(buffer);
  ...
  PTRACE(4, DINSK_CODEC_LOG, "H264\tCap\tCustom merge profile-level: " << src << " and " << dst << " to " << *result);
...
static int merge_packetization_mode(char ** result, const char * dst, const char * src)
{
  ...
  *result = strdup(buffer);
  ...
  PTRACE(4, DINSK_CODEC_LOG, "H264\tCap\tCustom merge packetization-mode: " << src << " and " << dst << " to " << *result);
...
unsigned int Opal_StaticCodec_DINSK_H264_GetAPIVersion()
...
struct PluginCodec_Definition * Opal_StaticCodec_DINSK_H264_GetCodecs(unsigned * p1, unsigned p2)
...
PLUGIN_CODEC_DLL_API struct PluginCodec_Definition * PLUGIN_CODEC_GET_CODEC_FN(unsigned * count, unsigned version)
{
  ...
  if (debug_level!=NULL) {
    PTrace::SetLevelUserPlane(atoi(debug_level));
  } 
  else {
    PTrace::SetLevelUserPlane(0);
  }
#endif // PTRACING

  if (version < PLUGIN_CODEC_VERSION_OPTIONS) {
    *count = 0;
    PTRACE(1, DINSK_CODEC_LOG, "H264\tCodec\tDisabled - plugin version mismatch");
    return NULL;
  }
  else {
    *count = sizeof(h264CodecDefn) / sizeof(struct PluginCodec_Definition);
    PTRACE(1, DINSK_CODEC_LOG, "H264\tCodec\tEnabled");
...
IActivation *gph264Activation = NULL;
